 ...
 * Used when CPU preference is cleared to force auto-selection.
 */
 ...
 *
 * Behavior:
 * - If not connected: connects to specified CPU or first available if not specified
...
export async function handleExecute(
  ...
  // Use queue() for clean output (strips echo, markers, control chars)
  return await conn.queue(input.command, input.timeout);
}
...
export const commandTool: ToolDefinition = {
  ...
  handler: async (args, ctx) => {
    try {
      ...
      // Use queue() for clean marker-based output extraction
      // This avoids echo stripping issues with cleanOutput()
      const result = await conn.queue(args.command as string, args.timeout as number);
