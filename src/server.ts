      ...
      try {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
          if (attempt > 1) {
            ...
            await conn.execute('SET ADDONS:MJ:NODE:ENABLED TO FALSE.');
            await conn.execute('SET THROTTLE TO 0. UNLOCK THROTTLE. UNLOCK STEERING.');
            await new Promise(resolve => setTimeout(resolve, 2000));
          }

          // Enable the node executor
          await conn.execute('SET ADDONS:MJ:NODE:ENABLED TO TRUE.');
          ...
          while (Date.now() - startTime < maxBurnTime) {
            ...
            const executorCheck = await conn.execute('PRINT ADDONS:MJ:NODE:ENABLED.');
